    ...
    if key==GLFW_KEY_ESCAPE and action==GLFW_PRESS:
        ...
        if action==GLFW_PRESS or action==GLFW_REPEAT:
            if key==GLFW_KEY_V:
                ...
                globalvar.g_box_rendering_mode = False
            elif key==GLFW_KEY_2:
                globalvar.g_box_rendering_mode = True
            elif key==GLFW_KEY_SPACE:
                globalvar.main_bvh_paused = not globalvar.main_bvh_paused
...
def drop_callback(window, path):
    ...
    # bvh parser
    globalvar.g_bvh_offset_multiplier = 1 #reset multiplier
    globalvar.g_box_rendering_mode = False
    (root, frames, frame_time, joint_count, joint_names) = bvh_parser(os.path.join(path[0]))
    print('Number of frames:', frames)
    print('FPS', 1/frame_time)
    print('Number of joints:', joint_count)
    print('List of all joint names: ')
    for name in joint_names:
        print(name)

    globalvar.main_bvh = root
    globalvar.main_bvh_frames = frames
    globalvar.main_bvh_frame_time = frame_time

    globalvar.main_bvh_curframe = -1
    globalvar.main_bvh_paused = True
